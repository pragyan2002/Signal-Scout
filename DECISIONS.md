...
## Static dashboard with embedded run data, deployed free on GitHub Pages

The pipeline's only outputs were a terminal table and `output.json` — nothing to *show*. Added a single self-contained `dist/index.html` (built by `scripts/build-site.ts`, `npm run build:site`) that renders the committed `runs/*.json` as a product-grade page: run-history timeline, per-prospect cards, and the anchor-post → opener as a "what they posted → your opener" before/after. Deployed via `.github/workflows/deploy.yml` to GitHub Pages — free, no backend, no keys at view-time. The deploy workflow also fires on `workflow_run` after `run-pipeline`, so each daily run auto-refreshes the hosted page.

The build **embeds all run data into the HTML** as a `<script type="application/json">` blob rather than fetching at runtime. No runtime fetch means no base-path, CORS, or Jekyll surprises on Pages, and the file renders identically from `file://` — verification is "open the file." `<` is escaped in the embedded JSON and all run-derived strings render via `textContent`/`esc()`, since live Bluesky post text is attacker-influenceable. Stays consistent with "no database — every file is input, code, or output": `dist/` is build output (gitignored), not committed state.

Rejected:

- **Vite/React build.** Adds a bundler, `node_modules` at build, and a `base: '/signal-scout/'` footgun for zero benefit at ~80 KB of data. Vanilla + embedded data is lower-resistance and more robust.
- **Committing `/docs` for Pages.** Pollutes history with generated HTML on every run. Artifact deploy via `actions/deploy-pages` keeps the repo clean (`dist/` already gitignored).
- **Runtime `fetch` of `runs/*.json` + a manifest.** A second moving part and the classic source of "works locally, blank on Pages" base-path bugs. Embedding sidesteps all of it.

## Expand to multiple live Bluesky prospects; sharpen the snippet examples

Added several real, active Bluesky accounts (`jay.bsky.team`, `danabra.mov`, `bnewbold.net`, `why.bsky.team`) alongside the hand-tuned fixtures. The fixtures still carry the demo's strongest, on-ICP buying signals (fundraising, hiring, tool complaints); the live accounts prove the pipeline runs against real, unscripted posts — the dashboard labels which are live. New handles can't be verified offline, so the first real `run-pipeline` validates them; prune any that return zero posts or an error.

Also tightened `SNIPPET_SYSTEM_PROMPT`: replaced the single example with three calibrated bad/good pairs (migration, fundraising, tool-complaint) and rewrote rule 2 to demand a *second-order claim about what the post implies* rather than a summary, plus banned the limp hedges ("suggests a common", "highlights a", "is a concern for you") that 70B models reach for when summarizing instead of thinking. Kept the existing guardrails (banned-phrase retry, anti-fabrication fallback, <25-word cap). Prompt-only change — no extra verifier LLM call, consistent with the earlier decision to fix generation rather than add a grading pass.

Rejected:

- **Replacing the fixtures entirely with live accounts.** Bluesky's population skews dev/tech, not founders mid-outbound, so pure-live output risks flat, off-ICP openers. Blending keeps the persuasive demo and the credibility of real data.
- **A second LLM pass to grade/rewrite weak openers.** More cost, more latency, its own failure modes. Tightening the first-pass prompt is cheaper and already the project's stated approach.